...
### 2-2. 힌트 
정수 5개를 요소로 갖는 배열을 생성해 삽입 정렬 방식으로 구현해보세요. 다음과 같은 순서로 구분하면 됩니다.
1. 프로젝트 생성
2. 삽입 정렬 함수 insertSort() 구현
3. main() 함수에서 정렬 결과 출력

#### 프로그램 구조
![421_quiz_hint](https://github.com/user-attachments/assets/79be103e-8421-463a-8975-8af0cbc96002)

#### 전체코드
![422_quiz_code](https://github.com/user-attachments/assets/00dc70a5-f5e3-4e5a-bb55-30dd7bddb17b)
![423_quiz_code_result](https://github.com/user-attachments/assets/e55c54c8-5a88-4d83-aabd-5129e8339f48)

### 학습 마무리 - 핵심 요약 
#### 1. 버블 정렬 알고리즘 동작 규칙
1-1. 인접한 요소 끼리 비교하여 더 작은 값을 앞으로, 큰 값을 뒤로 저장합니다.

1-2. 1번 과정을 마지막 원소까지 반복합니다.

1-3. 더이상 자리를 교환하지 않을 때까지 1번 2번을 반복합니다.

#### 2. 삽입 정렬 알고리즘 동작 규칙
2-1. 인덱스 0은 정렬되어 있다고 가정합니다. 그래서 현 위치는 인덱스 1부터 시작합니다. 

2-2. 현 인덱스의 값과 바로 앞 인덱스의 값을 비교하여 더 작은 값을 앞으로, 큰 값을 뒤로 저장합니다. 

2-3. 2번의 과정을 반복하다가 바로 앞에 있는 값이 더 작다면 현 위치를 뒤로 옮깁니다(인덱스 +1 위치로).

2-4. 마지막 인덱스까지 2번 3번을 반복합니다.
